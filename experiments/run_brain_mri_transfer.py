...
def _save_transfer_validation_curves(results: list[dict], output_dir: Path) -> None:
    ...
    with plt.rc_context(PLOT_STYLE):
        ...
        figure.savefig(output_dir / "brain_mri_transfer_validation_curves.png", dpi=240)
        plt.close(figure)
...
def _save_downstream_checkpoint(
    ...
    checkpoint_path = output_dir / f"{model_name}_{initialization}_{adaptation}_{config.brain_mri.slug}_best.pt"
    checkpoint = {
...
def run_brain_mri_transfer(config: ProjectConfig, device: torch.device) -> dict:
    ...
    if config.brain_transfer.adaptation_mode == "both":
        adaptation_modes = ("linear_probe", "full_finetune")
    elif config.brain_transfer.adaptation_mode == "linear_probe":
    ...
    if "pretrained" in run_modes:
        explicit_checkpoint_paths = {
            ...
            "dhvt": config.brain_transfer.dhvt_checkpoint,
        }
