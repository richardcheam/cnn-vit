...
\begin{abstract}
This report summarizes the current saved results of the project. The study starts with a controlled CIFAR-10 comparison between a lightweight CNN and a lightweight ViT, then transfers both models to EuroSAT and Brain Tumor MRI. The main findings are direct: the CNN is stronger on the source CIFAR-10 task, especially in low-data training, while the ViT is more robust to texture corruption. In downstream transfer, linear probing is clearly weaker than full fine-tuning. Full fine-tuning gives only a small gain on EuroSAT but a strong gain on Brain Tumor MRI. The best downstream result so far is the pretrained ViT with full fine-tuning on Brain Tumor MRI.
\end{abstract}
...
\toprule
Model & Setup & Acc. & Macro Prec. & Macro Recall & Macro-F1 & Time \\
\midrule
...
\begin{itemize}
    \item linear probing is much weaker than full fine-tuning for both architectures
    \item the ViT is especially weak under a frozen backbone: 79.00\% with linear probing versus 94.15\% with full fine-tuning
    \item full fine-tuning gives only a small gain over scratch
    \item CNN full fine-tuning remains the strongest EuroSAT result
\end{itemize}

This suggests that CIFAR features alone are not enough for strong EuroSAT transfer; downstream adaptation is important. At the same time, EuroSAT is easy enough that scratch training is already strong, so the final gain from full fine-tuning remains small.
...
\begin{itemize}
    ...
    \item this drop is severe for both models, which indicates that downstream adaptation is necessary rather than optional
    \item full fine-tuning gives a large gain over scratch
    \item pretrained ViT with full fine-tuning is the strongest downstream result in the repository
\end{itemize}
...
\begin{itemize}
    ...
    \item CIFAR pretraining matters much more for Brain Tumor MRI than for EuroSAT, and the pretrained ViT with full fine-tuning is currently the best medical downstream model in the repository.
\end{itemize}
