...
\begin{abstract}
This report summarizes the current saved results of the project. The study frames architecture comparison, frugal learning, transfer learning, and explainable AI in one pipeline. It begins with a controlled CIFAR-10 comparison between a lightweight CNN, a lightweight ViT, and a compact DHVT baseline, then transfers all three architectures to EuroSAT and Brain Tumor MRI under scratch training, linear probing, and full fine-tuning. The main findings are direct: DHVT is the strongest clean CIFAR-10 classifier, the CNN remains competitive in the low-data regime, and the vanilla ViT is the most robust to texture corruption. In downstream transfer, linear probing is consistently weaker than full fine-tuning. On EuroSAT, strong scratch training is already sufficient and DHVT scratch is best. On Brain Tumor MRI, CIFAR initialization helps substantially, and pretrained DHVT with full fine-tuning is the strongest downstream model.
\end{abstract}

\section{Introduction}
CNNs use locality and weight sharing as inductive biases \cite{lecun1998gradient}. ViTs instead model patch interactions with self-attention \cite{dosovitskiy2021image}. Prior work shows that ViTs usually require more data or stronger training recipes \cite{dosovitskiy2021image,touvron2021training}, while standard CNNs often rely more on local texture cues \cite{geirhos2019imagenet}. DHVT was proposed to bridge this gap on small datasets by combining transformer-style token mixing with stronger convolutional inductive bias \cite{lu2022bridging}. This project studies these differences as an architecture-comparison problem, a frugal-learning problem, and an explainable-AI problem at the same time. The pipeline therefore asks not only which model is most accurate, but also which model is more data-efficient, which transfer strategy is cheaper, and whether the explanation maps support the final prediction. These questions are addressed in two stages: first through a controlled CIFAR-10 source experiment with robustness and interpretability analysis, and then through downstream transfer to EuroSAT and Brain Tumor MRI.
...
The repository models are lightweight research baselines, not exact reproductions of one published architecture. The CNN, implemented in \texttt{models/cnn.py}, is a custom compact baseline inspired by LeNet-style convolution / pooling classifiers \cite{lecun1998gradient}, VGG-style stacked small convolutions \cite{simonyan2015very}, and Batch Normalization \cite{ioffe2015batch}. The ViT, implemented in \texttt{models/vit.py}, is a compact custom implementation inspired by the Transformer encoder \cite{vaswani2017attention} and the original Vision Transformer design \cite{dosovitskiy2021image}. The DHVT model, implemented in \texttt{models/dhvt.py}, is a compact adaptation of the official DHVT design \cite{lu2022bridging}, following the paper and official repository components such as convolutional patch embedding, head-token interaction attention, and the dynamic adaptive feed-forward block. This wording is important for attribution: the code is custom, while the architectural ideas and adaptation targets are grounded in these published sources and the official DHVT repository.
